...
clienteDAO clientedao = new clienteDAO();
	...
	avanteVcAguardandoInstalacao();
	//clientedao.addClientes(clientedao.getClientes());
	URL website = new URL("http://avante.com/intranet/microcredits_xls?action=report&controller=intranet%2Forders&format=xls");
	ReadableByteChannel rbc = Channels.newChannel(website.openStream());
	FileOutputStream fos = new FileOutputStream("/home/information.xls");
	fos.getChannel().transferFrom(rbc, 0, Long.MAX_VALUE);
	
	}

	public static void avanteVcPagamentosRealizados(List<clienteBean> a) throws Exception {
		...
		pagamentos.add(b.getParcela());
		//pagamentos.add(8);
		extrator.setWebsite(new AvanteVcPagamentosRealizados(b.getCpf(), pagamentos));
	...
	public static void avanteVcAguardandoInstalacao() throws Exception {
		System.out.println("Start PegaRelatorio");
		// ExtratorInformacoesURL é o 'motor' que fará a extração de dados, bem burro.
		ExtratorInformacoesURL extrator = new ExtratorInformacoesURL();
		// parametros: cpf, código serial da máquina
		//for(clienteMaquininhaBean b : a){
		extrator.setWebsite(new AvanteVcAguardandoInstalacao("37325569807","213212"));
		...
		//}
		System.out.println("*----------------------*");
		System.out.println("Finish PegaRelatorio");
	}
