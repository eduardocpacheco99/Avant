...
public class InboxReader {  
    ...
    public static void main(String args[]) throws Exception {  
        ...
        try {  
            ...
            store.connect("imap.gmail.com", "parceiros.sistema@avante.com.vc", "DadosSoroCC!");   
...
//escreverEmail.setParceiro(null);
from2 = "stone";//
escreverEmail.setParceiro("stone");
}
...
//        	escreverEmail.setParceiro(from2);
 ...
 //     if(messagePart.getContentType())
            ...
            String attachFiles = "";
            String caminhoBase = "/home/ubuntu/pentaho/files/";  
            ...
            if (contentType.contains("multipart")) {
                ...
                for (int partCount = 0; partCount < numberOfParts; partCount++) {	
                    ...
                    if (Part.ATTACHMENT.equalsIgnoreCase(part.getDisposition())) {
                        ...
                        // determina a categoria pelo nome do arquivo
                        if(fileName.contains("Faturamento")){
                        	escreverEmail.setCategoria("faturamento");
                        }else if (fileName.contains("Credenciamento")){
                        	escreverEmail.setCategoria("credenciamento");
                        }else if (fileName.contains("stica")){
                        	escreverEmail.setCategoria("logistica");
                        }else if (fileName.contains("Relat")){
                        	escreverEmail.setCategoria("geral");
                        }else if (fileName.contains("Pagamento")){
                        	escreverEmail.setCategoria("parcelaspagas");
                        	escreverEmail.setParceiro("omni");

                        }else{
                        	escreverEmail.setCategoria("indefinido");
                        	
                        }
                        
                       System.out.println(escreverEmail.getCategoria()); 
                        //determina o mes de referencia
                    String mes;
                    String dia;
                       if(!escreverEmail.getCategoria().equals("geral") && !escreverEmail.getCategoria().equals("parcelaspagas")){
                       mes =  fileName.split(".xls")[0].substring(fileName.split(".xls")[0].length()-2, fileName.split(".xls")[0].length());
                       dia =  fileName.split(".xls")[0].substring(fileName.split(".xls")[0].length()-4, fileName.split(".xls")[0].length()-2);
                       
                       }else if(escreverEmail.getCategoria().equals("parcelaspagas")){
                    	 	 mes =  fileName.split(".xls")[0].substring(fileName.split(".xls")[0].length()-6, fileName.split(".xls")[0].length()-4);
                             dia =  fileName.split(".xls")[0].substring(fileName.split(".xls")[0].length()-8, fileName.split(".xls")[0].length()-6);
                                
System.out.println(mes+dia);
}else{
                    	 mes =  fileName.split(".xls")[0].substring(fileName.split(".xls")[0].length()-4, fileName.split(".xls")[0].length()-2);
                         dia =  fileName.split(".xls")[0].substring(fileName.split(".xls")[0].length()-6, fileName.split(".xls")[0].length()-4);
                          
                    	   
                       }
                       System.out.println(mes);
                      escreverEmail.setMes(mes);
                      escreverEmail.setDia(dia);
